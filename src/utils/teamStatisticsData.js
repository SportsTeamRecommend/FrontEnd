// f1

// 이미지들
import ferrari from './../assets/f1-logo/ferrari.svg';
import mcLaren from './../assets/f1-logo/mclaren.svg';
import mercedes from './../assets/f1-logo/mercedes.svg';
import redbull from './../assets/f1-logo/redbull.svg';
import astonmartin from './../assets/f1-logo/astonmartin.png';
import alpine from './../assets/f1-logo/alpine.svg';
import haas from './../assets/f1-logo/haas.svg';
import sauber from './../assets/f1-logo/sauber.svg';
import williams from './../assets/f1-logo/williams.svg';
import rb from './../assets/f1-logo/rb.png';

export const f1TopTeams = [
  {
    rank: 1,
    icon: '🥇',
    img: ferrari,
    teamName: 'Ferrari',
    recommendations: '1,582',
    likes: 31.2,
    color: '#ED1131',
  },
  {
    rank: 2,
    icon: '🥈',
    img: redbull,
    teamName: 'Red Bull Racing',
    recommendations: '1,247',
    likes: 26.7,
    color: '#1E41FF',
  },
  {
    rank: 3,
    icon: '🥉',
    img: mcLaren,
    teamName: 'McLaren',
    recommendations: '980',
    likes: 19.8,
    color: '#F47600',
  },
  {
    rank: 4,
    icon: '4',
    img: mercedes,
    teamName: 'Mercedes-AMG Petronas',
    recommendations: '865',
    likes: 17.5,
    color: '#00D7B6',
  },
  {
    rank: 5,
    icon: '5',
    img: astonmartin,
    teamName: 'Aston Martin Aramco',
    recommendations: '742',
    likes: 15.1,
    color: '#229971',
  },
  {
    rank: 6,
    icon: '6',
    img: alpine,
    teamName: 'Alpine',
    recommendations: '621',
    likes: 12.3,
    color: '#00A1E8',
  },
  {
    rank: 7,
    icon: '7',
    img: williams,
    teamName: 'Williams',
    recommendations: '540',
    likes: 10.8,
    color: '#1868DB',
  },
  {
    rank: 8,
    icon: '8',
    img: rb,
    teamName: 'Racing Bulls',
    recommendations: '482',
    likes: 9.6,
    color: '#6C98FF',
  },
  {
    rank: 9,
    icon: '9',
    img: sauber,
    teamName: 'Kick Sauber (Stake F1 Team)',
    recommendations: '398',
    likes: 7.9,
    color: '#01C00E',
  },
  {
    rank: 10,
    icon: '10',
    img: haas,
    teamName: 'Haas F1 Team',
    recommendations: '354',
    likes: 7.0,
    color: '#9C9FA2',
  },
];

// kbo

// 이미지들
import kia from './../assets/kbo-logo/kia-tigers.svg';
import lg from './../assets/kbo-logo/lg-twins.svg';
import doosan from './../assets/kbo-logo/doosan-bears.svg';
import lotte from './../assets/kbo-logo/lotte-giants.svg';
import kiwoom from './../assets/kbo-logo/kiwoom-heroes.svg';
import hanwha from './../assets/kbo-logo/hanwha-eagles.svg';
import samsung from './../assets/kbo-logo/samsung-lions.svg';
import ssg from './../assets/kbo-logo/ssg-landers.svg';
import kt from './../assets/kbo-logo/kt-wiz.svg';
import nc from './../assets/kbo-logo/nc-dinos.svg';

export const kboTopTeams = [
  {
    rank: 1,
    icon: '🥇',
    img: kia,
    teamName: 'KIA Tigers',
    recommendations: '1,820',
    likes: 35.1,
    color: '#EA0029',
  },
  {
    rank: 2,
    icon: '🥈',
    img: lg,
    teamName: 'LG Twins',
    recommendations: '1,110',
    likes: 25.5,
    color: '#C30452',
  },
  {
    rank: 3,
    icon: '🥉',
    img: doosan,
    teamName: 'Doosan Bears (두산)',
    recommendations: '850',
    likes: 18.2,
    color: '#1A1748',
  },
  {
    rank: 4,
    icon: '4',
    img: lotte,
    teamName: 'Lotte Giants (롯데)',
    recommendations: '820',
    likes: 17.5,
    color: '#041E42',
  },
  {
    rank: 5,
    icon: '5',
    img: samsung,
    teamName: 'Samsung Lions (삼성)',
    recommendations: '780',
    likes: 16.9,
    color: '#074CA1',
  },
  {
    rank: 6,
    icon: '6',
    img: hanwha,
    teamName: 'Hanwha Eagles (한화)',
    recommendations: '750',
    likes: 15.2,
    color: '#FC4E00',
  },
  {
    rank: 7,
    icon: '7',
    img: ssg,
    teamName: 'SSG Landers',
    recommendations: '710',
    likes: 14.8,
    color: '#CE0E2D',
  },
  {
    rank: 8,
    icon: '8',
    img: nc,
    teamName: 'NC Dinos',
    recommendations: '680',
    likes: 13.5,
    color: '#315288',
  },
  {
    rank: 9,
    icon: '9',
    img: kt,
    teamName: 'KT Wiz',
    recommendations: '650',
    likes: 12.1,
    color: '#000000',
  },
  {
    rank: 10,
    icon: '10',
    img: kiwoom,
    teamName: 'Kiwoom Heroes (키움)',
    recommendations: '610',
    likes: 11.5,
    color: '#570514',
  },
];
